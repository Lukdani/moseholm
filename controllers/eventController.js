...
export class EventController {
    constructor(eventModel, eventView, showOrderBy = false) {
        ...
        if (this.showOrderBy) {
            ...
            this.eventView.bindOrderByButton((orderBy) => {this.populateEvents(orderBy); console.log(orderBy)});
        }

        this.populateEvents(null, true);
    }
     ...
     populateEvents = async (orderBy, initialLoad) => {
         if (orderBy && orderBy !== this.orderBy) {
            ...
            this.ascOrDesc = this.ascOrDesc === "ASC" ? "DESC" : "ASC";
        }
        ...
        const fetchtedEvents =  await fetchJsonApi(`/moseholm/api/getEvents.php${orderBy ? "?orderBy=" + orderBy : ""}${this.ascOrDesc ? "&direction=" + this.ascOrDesc : ""}`);

        if (initialLoad) this.setSpinner(false);
        this.eventModel.setEvents(fetchtedEvents);
        this.eventView.renderEvents(this.eventModel.state.events, this.showOrderBy, initialLoad);
