...
export class EventView {
    ...
    renderEvents = (events) => {
        ...
        if (events?.length > 0) {
            ...
            const eventContainer = createElement("div", ["col-12", `col-lg-${this.widthOnLarge}`, "event-container"], null);
            ...
            eventDate.textContent = eventItem.eventDate;
...
renderSortButtons = () => {
    const eventContainer = document.getElementById("orderByContainer");
        const orderByDateButton = createElement("btn", ["btn", "product-category-button", "order-by-button"], null);
        ...
        const orderByPriceButton = createElement("btn", ["btn", "product-category-button", "order-by-button"], null);
...
handleOrderBy = (event, buttonElement, callback) => {
    if (event.target.getAttribute("data-orderby") === buttonElement.getAttribute("data-orderby")) {
    ...
    bindOrderByButton = (callback) => {
        ...
        orderByButtons?.forEach(orderByButtonItem => {
            orderByButtonItem.addEventListener("click", (e) => this.handleOrderBy(e, orderByButtonItem, callback));
        })
    }
}